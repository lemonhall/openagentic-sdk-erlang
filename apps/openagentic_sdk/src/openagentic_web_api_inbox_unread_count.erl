-module(openagentic_web_api_inbox_unread_count).
...
init(Req0, State0) ->
  ...
  case openagentic_case_store:list_inbox(SessionRoot, #{status => <<"unread">>}) of
    {ok, Mail} -> reply_json(200, #{unread_count => length(Mail)}, Req0, State);
