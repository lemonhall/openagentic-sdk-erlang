-module(openagentic_anthropic_messages_http).
...
httpc_data_dir() ->
  case os:getenv("OPENAGENTIC_HTTPC_DATA_DIR") of
    ...
    V -> openagentic_anthropic_messages_utils:to_list(V)
  end.

configure_proxy() ->
  ProxyUrl = first_env(["HTTPS_PROXY", "HTTP_PROXY", "https_proxy", "http_proxy"]),
  case ProxyUrl of
    false ->
      ok;
    Url ->
...
parse_proxy_url(Url0) ->
  Url = openagentic_anthropic_messages_utils:to_list(Url0),
  try
