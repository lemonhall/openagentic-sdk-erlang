-module(openagentic_web_api_reconsideration_packages_create).
...
init(Req0, State0) ->
  ...
  PackId = cowboy_req:binding(pack_id, Req0),
  {ok, BodyBin, Req1} = cowboy_req:read_body(Req0),
  case decode_json(BodyBin) of
    {ok, Obj0} ->
      Payload = (ensure_map(Obj0))#{case_id => CaseId, pack_id => PackId},
      case openagentic_case_store:create_reconsideration_package(SessionRoot, Payload) of
        {ok, Res} -> reply_json(201, Res, Req1, State);
        {error, inspection_not_ready} -> reply_json(409, #{error => <<"inspection_not_ready">>}, Req1, State);
