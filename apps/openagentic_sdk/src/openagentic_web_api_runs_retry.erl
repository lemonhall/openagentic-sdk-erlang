-module(openagentic_web_api_runs_retry).
...
init(Req0, State0) ->
  ...
  RunId = cowboy_req:binding(run_id, Req0),
  {ok, BodyBin, Req1} = cowboy_req:read_body(Req0),
  case decode_json(BodyBin) of
    {ok, Obj0} ->
      Obj = ensure_map(Obj0),
      Payload = Obj#{case_id => CaseId, task_id => TaskId, run_id => RunId},
      case openagentic_case_store:retry_run(SessionRoot, Payload) of
        ...
        {error, ready_to_activate} -> reply_json(409, #{error => <<"ready_to_activate">>}, Req1, State);
        {error, rectification_required} -> reply_json(409, #{error => <<"rectification_required">>}, Req1, State);
        {error, paused} -> reply_json(409, #{error => <<"paused">>}, Req1, State);
        {error, no_task_version} -> reply_json(409, #{error => <<"no_task_version">>}, Req1, State);
        {error, run_in_progress} -> reply_json(409, #{error => <<"run_in_progress">>}, Req1, State);
        {error, run_already_completed} -> reply_json(409, #{error => <<"run_already_completed">>}, Req1, State);
