-module(openagentic_web_api_tasks_revise).
...
init(Req0, State0) ->
  ...
  case decode_json(BodyBin) of
    {ok, Obj0} ->
      ...
      case openagentic_case_store:revise_task(SessionRoot, Payload) of
        {ok, Res} -> reply_json(201, Res, Req1, State);
        {error, governance_session_missing} -> reply_json(409, #{error => <<"governance_session_missing">>}, Req1, State);
        {error, governance_session_mismatch} -> reply_json(409, #{error => <<"governance_session_mismatch">>}, Req1, State);
