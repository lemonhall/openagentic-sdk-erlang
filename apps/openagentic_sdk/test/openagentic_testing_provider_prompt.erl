-module(openagentic_testing_provider_prompt).
  ...
  case Step of
    0 ->
      ...
      {ok, #{
        assistant_text => <<>>,
        tool_calls => [
          %% Use an unknown tool name so execution is harmless, but still triggers PermissionGate prompt.
          #{tool_use_id => <<"call_1">>, name => <<"NotARealTool">>, arguments => #{}}
        ],
        response_id => <<"resp_1">>,
        usage => #{}
      }};
