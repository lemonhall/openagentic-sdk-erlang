...
## 3. 本 Phase 的落地目标与边界
### 3.1 本阶段要解决什么
- 把散落的事实材料按 `observation_pack` 聚起来，形成“能否复议”的触发单元。
- 让检察官对整包材料形成 `inspection_review`，并主整理 `controversies`。
- 让系统生成可预览、可 deferred、可 superseded、可 consumed 的 `reconsideration_package`。
- 让用户走完“先阅卷，再决定是否开启复议”的闭环。

### 3.2 本阶段的实现边界
- 重点做：观察包、检察快照、复议卷宗、内邮通知、预览页动作、`deferred` 生命周期。
- 暂不要求：更强的审计聚合、统一时间线、全量索引重建工具。

### 3.3 本阶段 DoD
- 能按观察包聚合相关任务材料。
- 能生成 `inspection_review` 与争议清单。
- 能生成冻结的 `reconsideration_package`。
- 能支持 `开启复议` 与 `继续观察`。
- 能正确维护 `deferred / superseded / consumed_by_round` 链条。

### 3.4 对应原稿章节
- 检察、复议、争议与卷宗机制：§§9-10、§§21-36。
