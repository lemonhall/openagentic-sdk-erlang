...
- [ ] 让 `schedule_policy` 真正驱动 `monitoring_run` 生成
- [ ] 为 run 注入 task workspace、历史执行摘要、最近异常/整改摘要
- [ ] 为单次失败生成标准异常简报与异常 mail
- [ ] 扩充 `failure_class` 分类并在长期失败统计中使用
- [ ] 将 `report_contract` 升级为真正的交卷验收逻辑
- [ ] 在 task detail API 中返回 `run_attempts`
- [ ] 在 task detail API 中返回 `fact_reports`
- [ ] 在前端展示 attempt / report / transcript / report lineage
- [ ] 细化 `monitoring_run` 与 `fact_report` 的生命周期状态
