...
截至 2026-03-07，本节列出的 `10.1 ~ 10.5` 已全部完成；这里保留的目的不再是“建议下一轮推进”，而是明确标记**哪些事项已经收口、具体收口到了哪里**。
...
### 10.7 Phase 1 收口停判据（已满足）
本轮已经按以下口径完成收口，不再处于“感觉差不多”的状态：
...
### 10.8 已完成的差距收口清单
下面这张表保留为“本轮已补齐了什么”的核对表，不再代表待办。

| 收口主题 | 保持不动的既有地基 | 本轮已完成内容 | 验收口径 |
| --- | --- | --- | --- |
| 模板制度 | `task` / `task_version` 上既有 `template_ref`、`derived_from_template_ref` 字段与实例独立 workspace | 已补模板对象、模板引用关系、模板实例化审计链；模板仅用于起草与参考实现 | 已能创建模板、从模板实例化候选任务、在任务/版本中追溯模板来源且不共享执行体 |
| 授权重接驳 | 既有 `credential_binding` 目录、绑定 API、激活 API、状态流 | 已补失效补录、轮换补录、重授权原因与动作沉淀 | 修订引入新凭证后，任务会进入明确待重授权状态；补录后可重新激活；全程有对象与状态证据 |
| 统一信箱 | 既有 `meta/mail/` 与 `mail-unread.json`，案卷页内已有 `mailList` | 已补统一信箱入口、已读/归档、状态筛选与全站跳转入口 | 已存在独立信箱入口，可查看未读、已读/归档并做状态过滤 |
| 审计硬化 | 既有统一对象外壳、`revision`、原子写盘与索引层 | 已补 `history.jsonl`、对象类型注册表、乐观并发校验 | 任一关键对象变更会留下轻量历史；对象类型可登记查询；并发写入冲突可被识别 |
| 治理体验收口 | 既有治理页、任务详情页、修订 API、差异提示与重授权提示 | 已完成第一版闭环；后续仅剩增强型结构化编辑体验 | 用户已能从治理页/详情页直接完成“修订 -> 看到差异 -> 补授权 -> 再激活”的主线操作 |
...
**结论先说：就本文件审计覆盖的 Phase 1 范围而言，已经完成对齐。当前不再存在必须继续补齐的 Phase 1 硬缺口。**

**现在还有没有需要做的？有，但已经不是“本轮没做完”，而是后续增强项。** 这些增强项包括：更复杂的结构化修订编辑体验、更细的信箱消息类型、以及超出本轮 DoD 的运行/检察/复议后续能力。
...
- 不要重做 `case` 主骨架。
- 不要重做基础 API。
- 把当前实现视为“Phase 1 已收口完成”，而不是“还差最后一点”。
- 后续如果继续做，只能按增强项展开，不要再把模板制度、统一信箱、审计硬化误判成未完成。
- 开工前先同时阅读：
  - 当前文件
  - `docs/plans/2026-03-06-case-monitoring-governance-and-reconsideration-phase-1-case-and-task-foundation.md`
  - `apps/openagentic_sdk/src/openagentic_case_store.erl`
...
| 模板制度 | 已对齐 | `openagentic_case_store.erl`、`openagentic_web_api_case_templates.erl`、`openagentic_web_api_case_template_instantiate.erl`、相关 EUnit |
| 统一信箱模型 | 已对齐 | `view/inbox.html`、`inbox.js`、`openagentic_web_api_inbox.erl`、`openagentic_web_api_case_mail_state.erl` |
| 审计硬化 | 已对齐 | `openagentic_case_store.erl` 中的 `history.jsonl` / 注册表 / 乐观并发校验，以及相关 EUnit |

这个表现在的用处只有一个：快速确认 **Phase 1 在本审计范围内已经收口**；后续如果继续做，请直接按增强项立新计划，不要再把这里列出的能力当作待补缺口。
