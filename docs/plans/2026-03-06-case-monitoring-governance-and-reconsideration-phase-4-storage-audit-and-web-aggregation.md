# 2026-03-06：Phase 4｜落盘审计、派生索引与 Web 聚合硬化
...
## 3. 本 Phase 的落地目标与边界
### 3.1 本阶段要解决什么
- 在前面几个 Phase 跑通之后，把对象落盘、并发保护、跨对象更新、审计记录与 Web 聚合补成稳定工程能力。
- 让系统具备可重建的派生索引、案卷级里程碑时间线和跨对象 `operation` 记录。
- 让 Web 查询不再依赖现场扫目录拼状态，而有稳定的聚合层。

### 3.2 本阶段的实现边界
- 重点做：三层分离、目录布局、公共头字段、派生索引、乐观并发、原子替换、`operation`、`timeline`、Web 聚合硬化。
- 不重做前面已确定的制度与对象边界；本阶段以工程一致性、审计性、可查询性为主。

### 3.3 本阶段 DoD
- 关键对象写入具备 `revision` + 乐观并发保护。
- 关键跨对象动作具备 `operation` 正式记录。
- `case` 级时间线可回放主要治理路径。
- Web 聚合查询不再依赖全量现场拼装。
- 派生层损坏后可从真相源重建。

### 3.4 对应原稿章节
- 落盘总原则、目录、并发、operation、timeline 与索引：§§37-53。
