...
## 3. 本 Phase 的落地目标与边界
### 3.1 本阶段要解决什么
- 把系统从“单次朝会”正式提升为以 `case` 为根的治理骨架。
- 把某轮 `deliberation_round` 与后续 `monitoring_task` 体系接起来。
- 让候选任务可以进入待审、经审定后转为正式任务，并生成首个 `task_version`。
- 为后续 Phase 2-4 留下稳定的对象模型、引用关系、治理会话与目录根。

### 3.2 本阶段的实现边界
- 重点做：`case`、`deliberation_round`、`monitoring_candidate`、`monitoring_task`、`task_version`。
- 重点做：模板起草与实例隔离、授权接驳分轨、治理会话沉淀。
- 暂不要求：真正的周期性执行链路、检察验卷、复议卷宗、时间线与派生索引硬化。

### 3.3 本阶段 DoD
- 能从某轮朝议建立 `case`。
- 能抽取候选任务并进入待审流。
- 能废弃或生效候选任务。
- 能为正式任务创建首个 `task_version`。
- 能把候选任务审议会话直接转正为长期 `governance_session_id`。
